...
export const addFaceAttributeModel:(path: string) => boolean;
...
export const updateFaceMeshList:(value: MeshType) => number;
export const get3DBeautyParts:() => st_effect_3D_beauty_part_info_t[];
export const set3dBeautyPartsStrength:(parts: st_effect_3D_beauty_part_info_t[]) => number;

export const changePackageBg:(packageId:number, image:st_image_t) => number;
export const getDetectResult:(image:st_image_t, rotate:RotateType, mirror: boolean) => DetectResult|null;

// st_mobile_human_action_detect 对外接口 - 提供完整检测结果
export const humanActionDetect:(image:st_image_t, rotate:RotateType, detectConfig:bigint, mirror: boolean) => HumanActionDetectResult|null;


export const getDetectConfig:() => bigint;
export const getCustomEventConfig:() => number;
export const setCustomEvent:(event:number)=> void;
...
export const setBodyBeauty:(param:BodyBeautyParam,path: string) => boolean;

// 多人美颜/美体接口 Multi-person beauty interfaces
export const setFaceBeautyStrengthByFaceId:(type:BeautyType, value: number, face_id: number) => boolean;
export const clearFaceBeautyStrengthByFaceId:(face_id: number) => boolean;
export const setBodyBeautyStrengthByBodyId:(type:BeautyType, value: number, body_id: number) => boolean;
export const clearBodyBeautyStrengthByBodyId:(body_id: number) => boolean;
export const getBodyBeautyStrengthByBodyId:(type:BeautyType, body_id: number) => number|null;
...
export const process:(srcBuffer: ArrayBuffer, format:PixelFormat,width:number,height:number,stride:number,rotate:RotateType,mirror:boolean,
  time_stamp:number, outBuffer:ArrayBuffer, outFormat:PixelFormat, orientation:RotateType, isOrigin:boolean,immediately:boolean,customEvent:number) => boolean;
...
//音效
export const setSoundLoadCallback: (callback: object) => number;
export const setSoundPlayCallback: (callback: object) => number;
export const setSoundStopCallback: (callback: object) => number;
export const setSoundPauseCallback: (callback: object) => number;
export const setSoundResumeCallback: (callback: object) => number;
...
export  const enum BodyBeautyParam{
...
/// \~chinese @brief 2D点坐标
/// \~english @brief 2D point coordinates
export interface Point2D {
  x: number;              ///< \~chinese 点的水平方向坐标 \~english The horizontal coordinate of the point
  y: number;              ///< \~chinese 点的竖直方向坐标 \~english The vertical coordinate of the point
}

/// \~chinese @brief 3D点坐标
/// \~english @brief 3D point coordinates
export interface Point3D {
  x: number;              ///< \~chinese 点的水平方向坐标 \~english The horizontal coordinate of the point
  y: number;              ///< \~chinese 点的竖直方向坐标 \~english The vertical coordinate of the point
  z: number;              ///< \~chinese 点的深度坐标 \~english The depth coordinate of the point
}
//检测结果
export interface DetectResult {
  ...
  faceShape:st_face_shape_t //脸型

  gender:string  //性别
  age:string     //年龄
  attractive:string  //颜值
}

// st_mobile_human_action_detect 检测结果结构体定义
/// \~chinese @brief 人脸检测结果
/// \~english @brief Face detection result
export interface FaceDetectInfo {
  rect: Rect;              ///< \~chinese 人脸矩形框 \~english Face bounding rectangle
  yaw: number;             ///< \~chinese 人脸yaw角度 \~english Face yaw angle
  pitch: number;           ///< \~chinese 人脸pitch角度 \~english Face pitch angle
  roll: number;            ///< \~chinese 人脸roll角度 \~english Face roll angle
  score: number;           ///< \~chinese 人脸置信度 \~english Face confidence score
  face_id: number;         ///< \~chinese 人脸ID \~english Face ID
}

/// \~chinese @brief 手部检测结果
/// \~english @brief Hand detection result
export interface HandDetectInfo {
  id: number;              ///< \~chinese 手的id \~english Hand ID
  rect: Rect;              ///< \~chinese 手部矩形框 \~english Hand bounding rectangle
  left_right: number;      ///< \~chinese 左手/右手 0未知,1左手,2右手 \~english Left/right hand: 0 unknown, 1 left, 2 right
  key_points?: Point2D[];  ///< \~chinese 手部关键点 \~english Hand key points
  key_points_count: number;///< \~chinese 手部关键点个数 \~english Number of hand key points
  hand_action: number;     ///< \~chinese 静态手势 \~english Hand action (see ST_HAND_ACTION_TYPE_* constants)
  score: number;           ///< \~chinese 静态手势置信度 \~english Confidence score of hand action
  skeleton_keypoints?: Point2D[];      ///< \~chinese 手部骨骼点 \~english Hand skeleton points
  skeleton_keypoints_count: number;    ///< \~chinese 手部骨骼点个数 一般是0/20 \~english Number of hand skeleton points, usually 0/20
  skeleton_3d_keypoints?: Point3D[];   ///< \~chinese 手部3d骨骼点 \~english 3D hand skeleton points
  skeleton_3d_keypoints_count: number; ///< \~chinese 手部3d骨骼点的个数，一般是0/21 \~english Number of 3D hand skeleton points, usually 0/21
}

/// \~chinese @brief 肢体检测结果
/// \~english @brief Body detection result
export interface BodyDetectInfo {
  id: number;                        ///< \~chinese 肢体id \~english Body ID
  key_points?: Point2D[];            ///< \~chinese 肢体关键点 \~english Body key points
  key_points_score?: number[];       ///< \~chinese 肢体关键点的置信度[0,1] \~english Confidence scores of body key points [0,1]
  key_points_count: number;          ///< \~chinese 肢体关键点个数 目前为0/4/14 \~english Number of body key points, currently 0/4/14
  contour_points?: Point2D[];        ///< \~chinese 肢体轮廓点 \~english Body contour points
  contour_points_score?: number[];   ///< \~chinese 肢体轮廓点的置信度[0,1] \~english Confidence scores of body contour points [0,1]
  contour_points_count: number;      ///< \~chinese 肢体轮廓点个数 目前为0/63 \~english Number of body contour points, currently 0/63
  key_points_3d?: Point3D[];         ///< \~chinese 肢体3d点 \~english Body 3D points
  key_points_3d_score?: number[];    ///< \~chinese 肢体3d点置信度 \~english Confidence scores of body 3D points
  key_points_3d_count: number;       ///< \~chinese 肢体3d点个数(0或17） \~english Number of body 3D points, currently 0/17
  label: number;                     ///< \~chinese 肢体label 0正常 1双手抱胸 2双手抱头 \~english Body label: 0 normal, 1 crossed arms, 2 hands on head
}

/// \~chinese @brief human_action 完整检测结果
/// \~english @brief Complete human action detection result
export interface HumanActionDetectResult {
  face_count: number;           ///< \~chinese 检测到的人脸数目 \~english Number of detected faces
  faces: FaceDetectInfo[];      ///< \~chinese 检测到的人脸信息数组 \~english Array of detected face information
  head_count: number;           ///< \~chinese 检测到的人头数目 \~english Number of detected heads
  hand_count: number;           ///< \~chinese 检测到的手的数目 \~english Number of detected hands
  hands?: HandDetectInfo[];     ///< \~chinese 检测到的手的信息数组 \~english Array of detected hand information
  body_count: number;           ///< \~chinese 检测到的肢体的数目 \~english Number of detected bodies
  bodys?: BodyDetectInfo[];     ///< \~chinese 检测到的肢体信息数组 \~english Array of detected body information
  camera_motion_score: number;  ///< \~chinese 摄像头运动状态置信度 \~english Camera motion score
}
/** 图像格式定义 | Image definition */
export interface st_image_t {
  /** 图像数据指针 | Image data pointer */
  data: ArrayBuffer;
  /** 像素格式 | Pixel format */
  ...
  height: number;
  /** 每行字节数 | Stride (bytes per row) */
  stride: number;
  /** 时间戳 | Time stamp */
  time_stamp: number;
}
/** 图像每个像素类型定义 | Pixel format definition */
...
export const enum st_face_shape_t {
  ...
  ST_FACE_SHAPE_RECTANGLE     ///< \~chinese 长方脸   \~english Bblong face
}
...
export const enum BeautyType{
  ...
  // \~Chinese 背景虚化
  BOKEH1_MODE = 0,//\~Chinese  虚化1
  BOKEH2_MODE = 1,// \~Chinese 虚化2
...
export const enum PixelFormat{
  ...
  ST_PIX_FMT_BGRA8888 = 4,///< BGRA 8:8:8:8 32bpp (
...
export interface st_effect_3D_beauty_part_info_t{
  name : string
  part_id: number
  strength: number
  strength_min: number
...
/// \~chinese @brief human_action 检测配置常量
/// \~english @brief Human action detection config constants
export const enum HumanActionDetectConfig {
  /// \~chinese 检测人脸 \~english Detect face
  ST_MOBILE_DETECT_FACE = 0x00000001,
  /// \~chinese 检测手势类别 \~english Detect hand gesture
  ST_MOBILE_DETECT_HAND_GESTURE = 0x00000200,
  /// \~chinese 检测人手 \~english Detect human hand
  ST_MOBILE_DETECT_HAND = 0x00000400,
  /// \~chinese 检测肢体 \~english Detect body
  ST_MOBILE_DETECT_BODY = 0x00000800,
  /// \~chinese 检测人脸240关键点 \~english Detect 240 face keypoints
  ST_MOBILE_DETECT_EXTRA_FACE_POINTS = 0x01000000,
  /// \~chinese 检测眼球中心点 \~english Detect eyeball center points
  ST_MOBILE_DETECT_EYEBALL_CENTER = 0x02000000,
  /// \~chinese 检测眼球轮廓点 \~english Detect eyeball contour points
  ST_MOBILE_DETECT_EYEBALL_CONTOUR = 0x04000000,
  /// \~chinese 检测舌头关键点 \~english Detect tongue keypoints
  ST_MOBILE_DETECT_TONGUE = 0x40000000,
}

/// \~chinese @brief 手势类型常量
/// \~english @brief Hand action gesture type constants
export const enum HandActionType {
  ST_HAND_ACTION_TYPE_OK = 0x00000200,              ///< \~chinese OK手势 \~english OK hand gesture
  ST_HAND_ACTION_TYPE_SCISSOR = 0x00000400,         ///< \~chinese 剪刀手 \~english Scissor hand gesture
  ST_HAND_ACTION_TYPE_GOOD = 0x00000800,            ///< \~chinese 大拇哥 \~english Thumbs up hand gesture
  ST_HAND_ACTION_TYPE_PALM = 0x00001000,            ///< \~chinese 手掌 \~english Palm gesture
  ST_HAND_ACTION_TYPE_PISTOL = 0x00002000,          ///< \~chinese 手枪手势 \~english Pistol hand gesture
  ST_HAND_ACTION_TYPE_LOVE = 0x00004000,            ///< \~chinese 爱心手势 \~english Heart hand gesture
  ST_HAND_ACTION_TYPE_HOLDUP = 0x00008000,          ///< \~chinese 托手手势 \~english Hold up hand gesture
  ST_HAND_ACTION_TYPE_FIST = 0x00200000,            ///< \~chinese 拳头手势 \~english Fist hand gesture
  ST_HAND_ACTION_TYPE_CONGRATULATE = 0x00020000,    ///< \~chinese 恭贺（抱拳）\~english Fist-palm salute
  ST_HAND_ACTION_TYPE_FINGER_HEART = 0x00040000,    ///< \~chinese 单手比爱心 \~english Finger heart gesture
  ST_HAND_ACTION_TYPE_FINGER_INDEX = 0x00100000,    ///< \~chinese 食指指尖 \~english Finger point gesture
  ST_HAND_ACTION_TYPE_666 = 0x00400000,             ///< \~chinese 666 \~english 666 hand gesture
  ST_HAND_ACTION_TYPE_BLESS = 0x00800000,           ///< \~chinese 双手合十 \~english Bless hand gesture
  ST_HAND_ACTION_TYPE_ILOVEYOU = 0x010000000000,    ///< \~chinese 手势ILoveYou \~english I love you hand gesture
  ST_HAND_ACTION_TYPE_THREE = 0x10000000000000,     ///< \~chinese 三根手指 \~english Three fingers
  ST_HAND_ACTION_TYPE_FOUR = 0x20000000000000,      ///< \~chinese 四根手指 \~english Four fingers
  ST_HAND_ACTION_TYPE_THUMBS_DOWN = 0x01,           ///< \~chinese 差评 \~english Thumbs down gesture
  ST_HAND_ACTION_TYPE_OTHER = 0,                    ///< \~chinese 其他手势 \~english Other gesture
}
