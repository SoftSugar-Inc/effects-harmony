...
static st_result_t module_state_change_cb(st_handle_t handle, const st_effect_module_info_t *module_info)
{
    ...
    else if (module_info->type == EFFECT_MODULE_SOUND) 
    {
        if(module_info->state == EFFECT_MODULE_LOADED){
            ...
            AudioManager::soundLoad(handle, ((st_effect_buffer_t*)module_info->reserved)->data_ptr, module_info->name, ((st_effect_buffer_t*)module_info->reserved)->data_len);
        } else if(module_info->state == EFFECT_MODULE_PLAYING){
            int loop = 0;
            if(module_info->reserved != nullptr){
                loop = reinterpret_cast<size_t>(module_info->reserved);
            }
            AudioManager::soundPlay(handle, module_info->name, loop);
        } else if(module_info->state == EFFECT_MODULE_PAUSED_LAST_FRAME){
            AudioManager::soundStop(handle, module_info->name);
        } else if(module_info->state == EFFECT_MODULE_PAUSED){
            AudioManager::soundPause(handle, module_info->name);
        } else if(module_info->state == EFFECT_MODULE_RESUMED){
            AudioManager::soundResume(handle, module_info->name);
        }
    }
    else if(module_info->type == EFFECT_MODULE_SEGMENT) 
    {
        if (module_info->rsv_type == EFFECT_RESERVED_SEGMENT_BASECOLOR && module_info->reserved)
        {
            const auto &mobileWrapperMap = GetMobileWrapperMap();
            auto it = mobileWrapperMap.find(handle); 
            if (it != mobileWrapperMap.end())
            {
                const uint32_t *reserved = static_cast<const uint32_t *>(module_info->reserved);
                it->second->setParam(ST_HUMAN_ACTION_PARAM_GREEN_SEGMENT_COLOR, *reserved);
            }
        }
        else
        {
            LOGE("unsupported reserved type: %d or reserved is null", module_info->rsv_type);
            return ST_E_INVALIDARG;
        }
    }
    else 
    {
        LOGE("unsupported module type: %d", module_info->type);
        return ST_E_FAIL;
    }
    return ST_OK;
}
...
{
#if DEBUG_LOG
    mLastFrameTime = std::chrono::time_point<std::chrono::high_resolution_clock>::min();
#endif
}
...
bool STMobileWrapper::Initialize(EffectsMode mode)
{
    ...
    if (mhDetect)
    {
        ...
        st_mobile_human_action_setparam(mhDetect, ST_HUMAN_ACTION_PARAM_BODY_LIMIT, 10);
...
bool STMobileWrapper::addSubModel(const char *path)
{
    ...
    if (ret != ST_OK)
    {
        LOGE("add sub model failed: %d  path: %s", ret, path);
        return false;
    }
...
bool STMobileWrapper::addSubModel(const unsigned char *buffer, unsigned int length)
{
    ...
    if (ret != ST_OK)
    {
        LOGE("add sub model failed: %d  buffer: %p, length: %d", ret, buffer, length);
        return false;
    }
...
int STMobileWrapper::changePackage(const char *buffer, unsigned int length)
{
    ...
    if (ret != ST_OK)
    {
        LOGE("change package faild, ret: %d", ret);
    }
    return pkgId;
}
...
int STMobileWrapper::updateFaceMeshList(int mesh_type)
{
    __CHECK_HUMAN_ACTION_HANDLE__
    __CHECK_EFFECT_HANDLE__
    
    st_mobile_mesh_info_t mesh_mouth_eye_list;
    memset(&mesh_mouth_eye_list, 0, sizeof(mesh_mouth_eye_list));
    int ret = st_mobile_human_action_get_mesh_info(mhDetect, (st_mobile_mesh_type)mesh_type, &mesh_mouth_eye_list);
    if (ret == ST_OK) {
         ret = st_mobile_effect_set_face_mesh_list(mhEffect, mesh_mouth_eye_list.p_mesh); 
    }
    return ret;
}
std::vector<st_effect_3D_beauty_part_info_t> STMobileWrapper::get3DBeautyParts()
{
    if (!mhEffect)
    {
        LOGE("need initlize effect firstly");
        return std::vector<st_effect_3D_beauty_part_info_t>{};
    }
    int length = 0;
    st_moobile_effect_get_3d_beauty_parts_count(mhEffect, &length);
    std::vector<st_effect_3D_beauty_part_info_t> part_info(length);
    st_mobile_effect_get_3d_beauty_parts(mhEffect, part_info.data(), length);
    return part_info;
}
int STMobileWrapper::set3dBeautyPartsStrength(const std::vector<st_effect_3D_beauty_part_info_t> &part_info)
{
    __CHECK_EFFECT_HANDLE__
    int ret = st_mobile_effect_set_3d_beauty_parts_strength(mhEffect, part_info.data(), part_info.size());
    return ret;
}

bool STMobileWrapper::getDetectResult(const st_effect_in_image_t *p_src, st_mobile_human_action_t* humanAction, st_mobile_attributes_t** faceAttributes,st_face_shape_t *p_face_shape)
{
     bool ret = false;
    
    ret =process(&p_src->image, p_src->rotate, ST_MOBILE_FACE_DETECT, humanAction);
    if (!ret) return ret;
    ret = process(&p_src->image, humanAction, faceAttributes);
     if (!ret) return ret;
    st_mobile_human_action_get_face_shape(mhDetect, humanAction->p_faces, p_face_shape);
    
    return ret;
}

int STMobileWrapper::changePackageBg(int packageId, st_image_t* st_image)
{
    __CHECK_EFFECT_HANDLE__
    
    st_result_t ret = ST_OK;
    st_effect_package_info_t package_info {};
    ret = st_mobile_effect_get_package_info(mhEffect, packageId, &package_info);
    int moduleCount = package_info.module_count;
    if (moduleCount <= 0)
        return ret;
    std::vector<st_effect_module_info_t> moduleInfos(moduleCount);
    ret = st_mobile_effect_get_modules_in_package(mhEffect, packageId, moduleInfos.data(), moduleInfos.size());
    if (ret != ST_OK || moduleInfos.empty())
    {
        LOGE("get module infos failed, ret: %d", ret);
        return ret;
    }

    auto moduleInfo = moduleInfos.begin();
    moduleInfo->rsv_type = EFFECT_RESERVED_IMAGE;
    moduleInfo->reserved = (void *)st_image;

    return st_mobile_effect_set_module_info(mhEffect, &*moduleInfo);
}
...
uint64_t STMobileWrapper::getTriggerActions() const
{
    ...
    uint64_t detectConfig = getDetectConfig();
     if ((detectConfig & ST_MOBILE_DETECT_HAND_GESTURE) > 0) {
        long config = detectConfig & ST_MOBILE_DETECT_HAND_GESTURE;
        st_mobile_effect_get_human_trigger_actions(mhEffect,config ,&triggerAction);
        
    } else {
        triggerAction = detectConfig;
    }
    //st_mobile_effect_get_human_trigger_actions(mhEffect, (getDetectConfig()& ST_MOBILE_DETECT_HAND_GESTURE), &triggerAction);
...
bool STMobileWrapper::setBodyBeauty(st_effect_body_beautify_aux_type_t param, const char *path)
{
    __CHECK_EFFECT_HANDLE__

    st_result_t ret = st_mobile_effect_set_body_beauty(mhEffect, param, path);
    if (ret != ST_OK)
    {
        LOGE("failed to set body beauty ret: %d", ret);
        return false;
    }
...
bool STMobileWrapper::setFaceBeautyStrength(st_effect_beauty_type_t param, float val, int face_id)
{
    __CHECK_EFFECT_HANDLE__

    st_result_t ret = st_mobile_effect_set_face_beauty_strength(mhEffect, param, val, face_id);
    if (ret != ST_OK)
    {
        LOGE("failed to set face beauty strength for face_id: %d, ret: %d", face_id, ret);
        return false;
    }
...
bool STMobileWrapper::clearFaceBeautyStrength(int face_id)
{
    __CHECK_EFFECT_HANDLE__

    st_result_t ret = st_mobile_effect_clear_face_beauty_strength(mhEffect, face_id);
    if (ret != ST_OK)
    {
        LOGE("failed to clear face beauty strength for face_id: %d, ret: %d", face_id, ret);
        return false;
    }
...
bool STMobileWrapper::setBodyBeautyStrengthByBodyId(st_effect_beauty_type_t param, float val, int body_id)
{
    __CHECK_EFFECT_HANDLE__

    st_result_t ret = st_mobile_effect_set_body_beauty_strength_by_body_id(mhEffect, param, val, body_id);
    LOGE("set body beauty strength for body_id: %d, value: %f, ret: %d", body_id, val, ret);
    
    if (ret != ST_OK)
    {
        LOGE("failed to set body beauty strength for body_id: %d, ret: %d", body_id, ret);
        return false;
    }
...
bool STMobileWrapper::clearBodyBeautyStrengthByBodyId(int body_id)
{
    __CHECK_EFFECT_HANDLE__

    st_result_t ret = st_mobile_effect_clear_body_beauty_strength_by_body_id(mhEffect, body_id);
    if (ret != ST_OK)
    {
        LOGE("failed to clear body beauty strength for body_id: %d, ret: %d", body_id, ret);
        return false;
    }
...
bool STMobileWrapper::getBodyBeautyStrengthByBodyId(st_effect_beauty_type_t param, float* val, int body_id)
{
    __CHECK_EFFECT_HANDLE__

    st_result_t ret = st_mobile_effect_get_body_beauty_strength_by_body_id(mhEffect, param, val, body_id);
    if (ret != ST_OK)
    {
        LOGE("failed to get body beauty strength for body_id: %d, ret: %d", body_id, ret);
        return false;
    }
...
bool STMobileWrapper::process(const st_effect_in_image_t *p_src, st_image_t *p_dst, st_rotate_type orientation, bool origin, bool immediately, int customEvent)
{
...
#if DEBUG_LOG
    if (mLastFrameTime == std::chrono::time_point<std::chrono::high_resolution_clock>::min()) 
    {
    ...
    else 
    {
        ...
        float duration = static_cast<float>(std::chrono::duration_cast<std::chrono::milliseconds>(currentTime - mLastFrameTime).count());
        LOGE("frame interval: %.2fms", duration);
    ...
    bool ret = processImpl(&src, p_dst, orientation, origin, immediately, customEvent);
...
bool STMobileWrapper::processImpl(const st_effect_in_image_t *p_src, st_image_t *p_dst, st_rotate_type orientation, bool origin, bool immediately, int customEvent)
{
    __CHECK_EFFECT_HANDLE__
    ...
    if (!origin)
    {
        ...
        st_effect_custom_param_t custom{};  
        in.p_custom_param = &custom;
        in.p_custom_param->event = customEvent;
        in.p_custom_param->front_camera = (orientation == ST_CLOCKWISE_ROTATE_270);
        ...
        // if (animal.count > 0)
        // {
        //     drawPoints(animal.p_animal_face_array->p_key_points, animal.p_animal_face_array->key_points_count, p_src->image.height, p_src->image.width, dst_tex.id);
        // }
    }
    ...
    if (p_dst)
    {
        ...
        if (mReadPixel == 0)
        {
            st_mobile_glutil_create_read_pixel_id(mhGLUtil, &mReadPixel, mMode == EffectsMode::VIDEO ? PIXEL_READ_PBO : PIXEL_READ_SYNC);
        }
