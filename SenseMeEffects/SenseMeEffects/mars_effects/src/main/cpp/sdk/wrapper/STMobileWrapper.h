...
public:
    ...
    int updateFaceMeshList(int mesh_type);
    std::vector<st_effect_3D_beauty_part_info_t> get3DBeautyParts();
    int set3dBeautyPartsStrength(const std::vector<st_effect_3D_beauty_part_info_t> &part_info);
    
    bool getDetectResult(const st_effect_in_image_t *p_src,st_mobile_human_action_t* humanAction, st_mobile_attributes_t** faceAttributes,st_face_shape_t *p_face_shape);
    
    
    int changePackageBg(int packageId, st_image_t* st_image);
    ...
    bool setBodyBeauty(st_effect_body_beautify_aux_type_t param, const char *path);

    // 多人美颜/美体接口 Multi-person beauty interfaces
    bool setFaceBeautyStrength(st_effect_beauty_type_t param, float val, int face_id);
    bool clearFaceBeautyStrength(int face_id);
    bool setBodyBeautyStrengthByBodyId(st_effect_beauty_type_t param, float val, int body_id);
    bool clearBodyBeautyStrengthByBodyId(int body_id);
    bool getBodyBeautyStrengthByBodyId(st_effect_beauty_type_t param, float* val, int body_id);

    std::vector<st_effect_beauty_info_t> getOverlappedBeautyInfo() const;

    bool process(const st_effect_in_image_t *p_src, st_image_t *p_dst, st_rotate_type orientation, bool origin = false, bool immediately = false, int customEvent = 0);
...
private:
    bool processImpl(const st_effect_in_image_t *p_src, st_image_t *p_dst, st_rotate_type orientation, bool origin = false, bool immediately = false, int processImpl =0);
