...
/// @brief device type
typedef enum {
    EFFECT_DEVICE_TYPE_METAL    ///< \~chinese Metal后端 \~english Metal backend
} st_effect_device_type_t;

/// \~chinese
/// @brief 设备类型及相关的参数
/// \~english
/// @brief device type and the param
typedef struct {
    st_effect_device_type_t type;   ///< \~chinise 设备类型 \~english device type
    void* user_data;                ///< \~chinese iOS使用metal后端时command queue信息 \~english when using the metal backend on iOS, the currently actived command queue.
} st_effect_device_info_t;

/// \~chinese
/// @brief 创建特效句柄
/// @param[in] config 素材生效模式
/// @param[in] device_info 设备信息，目前仅支持iOS使用metal后端时传入MTLCommandQueue
/// @param[out] handle 特效句柄指针
/// @return 成功返回ST_OK, 失败返回其他错误码, 错误码定义在st_mobile_common.h中, 如ST_E_FAIL等
/// \~english
/// @brief Create an effect handle
/// @param[in] config Material activation mode
/// @param[in] device_info for device information, currently, only iOS using the Metal backend is supported, where a MTLCommandQueue should be passed.
/// @param[out] handle Pointer to effect handle
/// @return Returns ST_OK if successful, otherwise returns other error codes. Error code definitions can be found in st_mobile_common.h, such as ST_E_FAIL etc.
ST_SDK_API st_result_t
st_mobile_effect_create_handle_ex(uint32_t config, const st_effect_device_info_t* device_info, st_handle_t* p_handle);
...
typedef enum {
    ...
    EFFECT_PARAM_PREFER_MEMORY_CACHE,       ///< \~chinese 倾向于空间换时间，传0的话，则尽可能清理内部缓存，保持内存最小。目前主要影响2D贴纸以及3D共享资源 \~english Prefer space for time. If 0 is passed, it will clean up the internal cache as much as possible to keep the memory at a minimum. Currently, it mainly affects 2D stickers and 3D shared resources.
...
typedef enum {
    ...
    EFFECT_RESOURCE_EXTERNAL                        = 51,    ///< \~chinese SDK外置资源：原本内置于SDK的文件与图片等资源改为由外部提供 \~english SDK external resource: resources originally embedded in the SDK (files, images, etc.) are now provided externally
    // \~chinese 基础美颜 base
    // \~english Basic beauty group
    ...
    EFFECT_BEAUTY_PLASTIC_SHRINK_RECTANGLE_FACE     = 339,  ///< \~chinese 长方脸瘦脸，[0, 1.0], 默认值0.0， 0.0不做瘦脸 \~english Shrink rectangle face, [0, 1.0], default value is 0.0, 0.0 means no face slimming effect for rectangle face
    EFFECT_BEAUTY_PLASTIC_SHRINK_SQUARE_FACE        = 340,  ///< \~chinese 方脸瘦脸，[0, 1.0], 默认值0.0， 0.0不做瘦脸 \~english Shrink square face, [0, 1.0], default value is 0.0, 0.0 means no face slimming effect for square face

    EFFECT_BEAUTY_GROUP_LIFT_RETOUCH_MODE           = 349,  ///< \~chinese 组合开关：提拉+平整区域(350-353, 141-144)，配合st_mobile_effect_set_beauty_mode使用，0关闭，非0开启 \~english Group switch for lift+retouch(350-353, 141-144); use with st_mobile_effect_set_beauty_mode, 0 disable, non-zero enable
    EFFECT_BEAUTY_PLASTIC_LIFT_FACE_CONTOUR         = 350,  ///< \~chinese 提拉脸部轮廓，[0, 1.0]，默认值为0 \~english Lift face contour, [0, 1.0], default value is 0
    EFFECT_BEAUTY_PLASTIC_LIFT_EYE_AREA             = 351,  ///< \~chinese 提拉眼周区域，[0, 1.0]，默认值为0 \~english Lift eye area, [0, 1.0], default value is 0
    EFFECT_BEAUTY_PLASTIC_LIFT_FACE_MIDDLE          = 352,  ///< \~chinese 提拉面中区域，[0, 1.0]，默认值为0 \~english Lift face middle, [0, 1.0], default value is 0
    EFFECT_BEAUTY_PLASTIC_LIFT_MOUTH_AREA           = 353,  ///< \~chinese 提拉嘴周区域，[0, 1.0]，默认值为0 \~english Lift mouth area, [0, 1.0], default value is 0

    // \~chinese 调整 tone
    // \~english Tone adjustment group
    EFFECT_BEAUTY_TONE_CONTRAST                     = 601,  ///< \~chinese 对比度, [-1.0, 1.0], 默认值0.05, 0.0不做对比度处理 \~english Contrast, [-1.0, 1.0], default value 0.05, 0.0 means no contrast adjustment
    EFFECT_BEAUTY_TONE_SATURATION                   = 602,  ///< \~chinese 饱和度, [-1.0,1.0], 默认值0.10, 0.0不做饱和度处理 \~english Saturation, [-1.0,1.0], default value 0.10, 0.0 means no saturation adjustment
    ...
    EFFECT_BEAUTY_TONE_BRIGHTEN                     = 609,  ///< \~chinese 亮度, [-1.0, 1.0], 默认值0.0, 0.0不做亮度处理 \~english Brightness, [-1.0, 1.0], default value 0.0 means no brightness adjustment

    // \~chinese 背景虚化，是单独的一个组，不与其他美颜组相互overlap
    // \~english bokeh group
    ...
    EFFECT_BEAUTY_MAKEUP_DOUBLE_EYELID              = 412,  ///< \~chinese 双眼皮 \~english Double eyelid
    EFFECT_BEAUTY_MAKEUP_AEGYO_SAL                  = 413,  ///< \~chinese 卧蚕 \~english Aegyo sal
    ...
    EFFECT_BEAUTY_GAN_SKIN                          = 140,  ///< \~chinese gan模型对脸部皮肤进行磨皮处理，需要通过 st_mobile_effect_set_beauty 接口设置gan模型 \~english The gan model performs skin smoothing on facial skin and requires setting up the gan model through the st_mobile_effect_set_beauty interface.

    EFFECT_BEAUTY_RETOUCH_FOREHEAD                  = 141,  ///< \~chinese 额头区域平整 \~english Forehead retouch ratio
    EFFECT_BEAUTY_RETOUCH_EYE_AREA                  = 142,  ///< \~chinese 眼周区域平整 \~english Eye area retouch ratio
    EFFECT_BEAUTY_RETOUCH_FACE_MIDDLE               = 143,  ///< \~chinese 中脸区域平整 \~english Mid-face retouch ratio
    EFFECT_BEAUTY_RETOUCH_MOUTH_AREA                = 144,  ///< \~chinese 嘴周区域平整 \~english Mouth area retouch ratio

    // \~chinese 肤质效果 (互斥效果, 请先设置模式)
    // \~english Skin Texture Effects (Mutually exclusive, set mode first)
    EFFECT_BEAUTY_SKIN_TEXTURE                      = 151,  ///< \~chinese 肤质总开关。不直接设置强度, 而是用于 st_mobile_effect_set_beauty_mode 函数, 模式为 st_effect_skin_texture_mode 。 \~english General switch for skin texture. Do not set strength directly. Use it with st_mobile_effect_set_beauty_mode and st_effect_skin_texture_mode_t.

    // \~chinese 肤质效果参数。设置前, 需先通过 EFFECT_BEAUTY_SKIN_TEXTURE 设置对应的肤质模式。
    // 奶油肌支持: 遮瑕、高光、美白
    // 水光肌支持: 遮瑕、高光、美白
    // 高清肤质支持: 遮瑕、立体、肌理
    // 雾面肌支持：遮瑕、哑光、肤色
    // \~english Skin Texture Effect Parameters. Before setting, a corresponding skin texture mode must be set via EFFECT_BEAUTY_SKIN_TEXTURE.
    // Creamy skin supports: Concealer, Highlight, Whiten
    // Glassy skin supports: Concealer, Highlight, Whiten
    // HD skin supports: Concealer, Contour, Texture Deatil
    // Matte skin supports: Concealer, Matte, Skintone
    EFFECT_BEAUTY_SKIN_TEXTURE_CONCEALER            = 152,  ///< \~chinese 遮瑕, [0,1.0], 默认0.0。适用于所有肤质模式。 \~english Concealer, [0,1.0], default 0.0. Applies to all skin texture modes.
    EFFECT_BEAUTY_SKIN_TEXTURE_HIGHLIGHT            = 153,  ///< \~chinese 高光, [0,1.0], 默认0.0。仅在奶油肌或水光肌模式下有效。 \~english Highlight, [0,1.0], default 0.0. Only effective in Creamy or Glassy skin mode.
    EFFECT_BEAUTY_SKIN_TEXTURE_WHITEN               = 154,  ///< \~chinese 美白, [0,1.0], 默认0.0。仅在奶油肌或水光肌模式下有效。 \~english Whitening, [0,1.0], default 0.0. Only effective in Creamy or Glassy skin mode.
    EFFECT_BEAUTY_SKIN_TEXTURE_CONTOUR              = 155,  ///< \~chinese 立体, [0,1.0], 默认0.0。仅在高清肤质模式下有效。 \~english Contour/Structure, [0,1.0], default 0.0. Only effective in HD skin mode.
    EFFECT_BEAUTY_SKIN_TEXTURE_DETAIL               = 156,  ///< \~chinese 肌理, [0,1.0], 默认0.0。仅在高清肤质模式下有效。 \~english Texture Detail, [0,1.0], default 0.0. Only effective in HD skin mode.
    EFFECT_BEAUTY_SKIN_TEXTURE_MATTE                = 157,  ///< \~chinese 哑光, [0,1.0], 默认0.0。仅在雾面肌模式下有效。 \~english Matte, [0,1.0], default 0.0. Only effective in Matte skin mode.
    EFFECT_BEAUTY_SKIN_TEXTURE_SKINTONE             = 158,  ///< \~chinese 肤色, [0,1.0], 默认0.0。仅在雾面肌模式下有效。 \~english Skintone, [0,1.0], default 0.0. Only effective in Matte skin mode.

    // \~chinese 美体
    // \~english body Beauty Group
    EFFECT_BEAUTY_BODY_WHOLE                        = 901,  ///< \~chinese 身体整体美化系数，默认为0.0，范围[-1.0, 1.0], 0.0为原始效果，[0， 1]为瘦身，[-1, 0]为增肥 \~english Overall body beautification coefficient, default is 0.0, range [-1.0, 1.0], 0.0 means original effect, [0, 1] means slimming, [-1, 0] means fattening
    EFFECT_BEAUTY_BODY_HEAD                         = 902,  ///< \~chinese 头部美化系数，默认为0.0，范围[-1.0, 1.0], 0.0为原始效果，[0， 1]为瘦头，[-1, 0]为大头 \~english Head beautification coefficient, default is 0.0, range [-1.0, 1.0], 0.0 means original effect, [0, 1] means slimming head, [-1, 0] means enlarging head
    EFFECT_BEAUTY_BODY_SHOULDER                     = 903,  ///< \~chinese 肩部美化系数，默认为0.0，范围[-1.0, 1.0], 0.0为原始效果，[0， 1]为瘦肩，[-1, 0]为宽肩 \~english Shoulder beautification coefficient, default is 0.0, range [-1.0, 1.0], 0.0 means original effect, [0, 1] means slimming shoulders, [-1, 0] means broadening shoulders
    EFFECT_BEAUTY_BODY_WAIST                        = 904,  ///< \~chinese 腰部美化系数，默认为0.0，范围[-1.0, 1.0], 0.0为原始效果，[0， 1]为瘦腰，[-1, 0]为粗腰 \~english Waist beautification coefficient, default is 0.0, range [-1.0, 1.0], 0.0 means original effect, [0, 1] means slimming waist, [-1, 0] means broadening waist
    EFFECT_BEAUTY_BODY_HIP                          = 905,  ///< \~chinese 臀部美化系数，默认为0.0，范围[-1.0, 1.0], 0.0为原始效果，[0， 1]为瘦臀，[-1, 0]为丰臀 \~english Hip beautification coefficient, default is 0.0, range [-1.0, 1.0], 0.0 means original effect, [0, 1] means perky hips, [-1, 0] means flat hips
    EFFECT_BEAUTY_BODY_LEG                          = 906,  ///< \~chinese 腿部美化系数，默认为0.0，范围[-1.0, 1.0], 0.0为原始效果，[0， 1]为瘦腿，[-1, 0]为粗腿 \~english Leg beautification coefficient, default is 0.0, range [-1.0, 1.0], 0.0 means original effect, [0, 1] means slimming legs, [-1, 0] means broadening legs
    EFFECT_BEAUTY_BODY_HEIGHT                       = 907,  ///< \~chinese 身高美化系数，默认为0.0，范围[0.0, 1.0], 0.0为原始效果，[0， 1]为增高 \~english Height beautification coefficient, default is 0.0, range [0.0, 1.0], 0.0 means original effect, [0, 1] means increasing height
    EFFECT_BEAUTY_BODY_NECK                         = 908,  ///< \~chinese 颈部美化系数，默认为0.0，范围[0.0, 1.0], 0.0为原始效果，[0， 1]为瘦脖子 \~english Neck beautification coefficient, default is 0.0, range [0.0, 1.0], 0.0 means original effect, [0, 1] means slimming neck
    EFFECT_BEAUTY_BODY_CHEST                        = 909,  ///< \~chinese 胸部美化系数，默认为0.0，范围[0.0, 1.0], 0.0为原始效果，[0， 1]为丰胸 \~english Chest beautification coefficient, default is 0.0, range [0.0, 1.0], 0.0 means original effect, [0, 1] means enhancing chest
    EFFECT_BEAUTY_BODY_SQUARE_SHOULDER              = 910,  ///< \~chinese 直角肩美化系数，默认为0.0，范围[0.0, 1.0], 0.0为原始效果，[0， 1]为直角肩 \~english Square shoulder beautification coefficient, default is 0.0, range [0.0, 1.0], 0.0 means original effect, [0, 1] means square shoulders
    EFFECT_BEAUTY_BODY_ARM                          = 911,  ///< \~chinese 手臂美化系数，默认为0.0，范围[-1.0, 1.0], 0.0为原始效果，[0， 1]为瘦手臂，[-1, 0]为粗手臂 \~english Arm beautification coefficient, default is 0.0, range [-1.0, 1.0], 0.0 means original effect, [0, 1] means slimming arms, [-1, 0] means broadening arms
    EFFECT_BEAUTY_BODY_NECK_LENGTH                  = 912,  ///< \~chinese 长颈美化系数，默认为0.0，范围[-1.0, 1.0], 0.0为原始效果，[0， 1]为长颈, [-1, 0]为短颈 \~english Neck length beautification coefficient, default is 0.0, range [0.0, 1.0], 0.0 means original effect, [0, 1] means lengthening neck, [-1, 0] means shortening neck
    EFFECT_BEAUTY_BODY_NECK_SLIM                    = 913,  ///< \~chinese 瘦颈美化系数，默认为0.0，范围[-1.0, 1.0], 0.0为原始效果，[0， 1]为瘦颈，[-1, 0]为粗颈 \~english Neck slim beautification coefficient, default is 0.0, range [0.0, 1.0], 0.0 means original effect, [0, 1] means slimming neck, [-1, 0] means broadening neck
    EFFECT_BEAUTY_BODY_SIDE_HIP                     = 914,  ///< \~chinese 侧臀美化系数，默认为0.0，范围[0.0, 1.0], 0.0为原始效果，[0， 1]为丰臀 \~english Side hip beautification coefficient, default is 0.0, range [0.0, 1.0], 0.0 means original effect, [0, 1] means perky side hips
    EFFECT_BEAUTY_BODY_STRAIGHT_BACK                = 915,  ///< \~chinese 背部美化系数，默认为0.0，范围[-1.0, 1.0], 0.0为原始效果，[0， 1]为向前直背，[-1, 0]为向后驼背 \~english Straight back beautification coefficient, default is 0.0, range [-1.0, 1.0], 0.0 means original effect, [0, 1] means straightening back forward, [-1, 0] means hunching back
    EFFECT_BEAUTY_BODY_FLAT_BELLY                   = 916,  ///< \~chinese 腹部美化系数，默认为0.0，范围[0.0, 1.0], 0.0为原始效果，[0， 1]为瘦肚子 \~english Flat belly beautification coefficient, default is 0.0, range [0.0, 1.0], 0.0 means original effect, [0, 1] means slimming belly

    EFFECT_BEAUTY_WRINKLE_NASOLABIAL_FOLDS          = 951,  ///< \~chinese 法令纹去除, [0.0, 1.0], 默认0.0。 \~english Remove nasolabial folds, [0.0, 1.0], default 0.0.
    EFFECT_BEAUTY_WRINKLE_FOREHEAD                  = 952,  ///< \~chinese 额头纹去除, [0.0, 1.0], 默认0.0。 \~english Remove forehead wrinkles, [0.0, 1.0], default 0.0.
    EFFECT_BEAUTY_WRINKLE_EYE_AROUND                = 953,  ///< \~chinese 眼周纹去除, [0.0, 1.0], 默认0.0。 \~english Remove eye around wrinkles, [0.0, 1.0], default 0.0.
    EFFECT_BEAUTY_WRINKLE_NECK                      = 954,  ///< \~chinese 颈纹去除, [0.0, 1.0], 默认0.0。 \~english Remove neck wrinkles, [0.0, 1.0], default 0.0.
    EFFECT_BEAUTY_WRINKLE_LIP                       = 955,  ///< \~chinese 唇纹去除, [0.0, 1.0], 默认0.0。 \~english Remove lip wrinkles, [0.0, 1.0], default 0.0.

} st_effect_beauty_type_t;


/// \~chinese @brief 美颜分组信息, 各个分组包含具体内容请查看st_effect_beauty_type_t
/// \~english @brief Beauty group information, see st_effect_beauty_type_t for more detailed group information
typedef enum {
...
typedef enum {
    ...
    EFFECT_LIPSTICK_GLOSSY,             ///< \~chinese 光泽 \~english glossy
...
/// \~chinese
/// @brief 按人脸设置美颜强度，支持 141-144、201-205、301-340、350-353 对应的参数
/// @param[in] handle 已初始化的特效句柄
/// @param[in] param 美颜类型
/// @param[in] val 强度
/// @param[in] face_id 人脸ID，可从检测结果的 face106.ID 获取
/// @return 成功返回ST_OK, 失败返回其他错误码
/// \~english
/// @brief Set beauty strength per face, supports params mapped to 141-144, 201-205, 301-340, and 350-353.
/// @param[in] handle Initialized effects handle.
/// @param[in] param Beauty type.
/// @param[in] val Intensity.
/// @param[in] face_id Face ID from detection (face106.ID).
/// @return Returns ST_OK on success, otherwise error codes.
ST_SDK_API st_result_t
st_mobile_effect_set_face_beauty_strength(st_handle_t handle, st_effect_beauty_type_t param, float val, int face_id);

/// \~chinese
/// @brief 清除按人脸设置的美颜强度覆盖，支持清除 141-144、201-205、301-340、350-353 的覆盖；清除后该人脸回退到全局强度
/// @param[in] handle 已初始化的特效句柄
/// @param[in] face_id 人脸ID，可从检测结果的 face106.ID 获取；传 -1 清除所有人脸覆盖
/// @return 成功返回ST_OK, 失败返回其他错误码
/// \~english
/// @brief Clear per-face beauty strength overrides for 141-144, 201-205, 301-340, and 350-353. After clearing, the target face falls back to global strength.
/// @param[in] handle Initialized effects handle.
/// @param[in] face_id Face ID from detection (face106.ID); pass -1 to clear overrides for all faces.
/// @return Returns ST_OK on success, otherwise error codes.
ST_SDK_API st_result_t
st_mobile_effect_clear_face_beauty_strength(st_handle_t handle, int face_id);

/// \~chinese
/// @brief 按人体设置美体强度，仅支持 EFFECT_BEAUTY_BODY_* 参数；EFFECT_BEAUTY_BODY_WHOLE/EFFECT_BEAUTY_BODY_HEIGHT 按全局强度处理
/// @param[in] handle 已初始化的特效句柄
/// @param[in] param 美体类型（EFFECT_BEAUTY_BODY_*）
/// @param[in] val 强度
/// @param[in] body_id 人体ID，可从检测结果 st_mobile_body_t::id 获取
/// @return 成功返回ST_OK, 失败返回其他错误码
/// \~english
/// @brief Set body beauty strength per body, only supports EFFECT_BEAUTY_BODY_* params; EFFECT_BEAUTY_BODY_WHOLE/EFFECT_BEAUTY_BODY_HEIGHT are treated as global strength.
/// @param[in] handle Initialized effects handle.
/// @param[in] param Body beauty type (EFFECT_BEAUTY_BODY_*).
/// @param[in] val Intensity.
/// @param[in] body_id Body ID from detection (st_mobile_body_t::id).
/// @return Returns ST_OK on success, otherwise error codes.
ST_SDK_API st_result_t
st_mobile_effect_set_body_beauty_strength_by_body_id(st_handle_t handle, st_effect_beauty_type_t param, float val, int body_id);

/// \~chinese
/// @brief 按人体清除美体强度覆盖，清除后回退到全局强度
/// @param[in] handle 已初始化的特效句柄
/// @param[in] body_id 人体ID，可从检测结果 st_mobile_body_t::id 获取；传 -1 清除所有人体覆盖
/// @return 成功返回ST_OK, 失败返回其他错误码
/// \~english
/// @brief Clear body beauty strength overrides by body ID. After clearing, target body falls back to global strength.
/// @param[in] handle Initialized effects handle.
/// @param[in] body_id Body ID from detection (st_mobile_body_t::id); pass -1 to clear overrides for all bodies.
/// @return Returns ST_OK on success, otherwise error codes.
ST_SDK_API st_result_t
st_mobile_effect_clear_body_beauty_strength_by_body_id(st_handle_t handle, int body_id);

/// \~chinese
/// @brief 按人体获取美体强度，仅支持 EFFECT_BEAUTY_BODY_* 参数；EFFECT_BEAUTY_BODY_WHOLE/EFFECT_BEAUTY_BODY_HEIGHT 返回全局强度
/// @param[in] handle 已初始化的特效句柄
/// @param[in] param 美体类型（EFFECT_BEAUTY_BODY_*）
/// @param[out] val 强度
/// @param[in] body_id 人体ID，可从检测结果 st_mobile_body_t::id 获取
/// @return 成功返回ST_OK, 失败返回其他错误码
/// \~english
/// @brief Get body beauty strength per body, only supports EFFECT_BEAUTY_BODY_* params; EFFECT_BEAUTY_BODY_WHOLE/EFFECT_BEAUTY_BODY_HEIGHT return global strength.
/// @param[in] handle Initialized effects handle.
/// @param[in] param Body beauty type (EFFECT_BEAUTY_BODY_*).
/// @param[out] val Intensity.
/// @param[in] body_id Body ID from detection (st_mobile_body_t::id).
/// @return Returns ST_OK on success, otherwise error codes.
ST_SDK_API st_result_t
st_mobile_effect_get_body_beauty_strength_by_body_id(st_handle_t handle, st_effect_beauty_type_t param, float* val, int body_id);
...
typedef enum {
    EFFECT_SMOOTH_FACE_ONLY     = 0,      ///< \~chinese 只对人脸做磨皮处理 \~english smooth only on face
    EFFECT_SMOOTH_FULL_IMAGE    = 1,      ///< \~chinese 对全图做磨皮处理 \~english whole image smooth
    EFFECT_SMOOTH_FACE_DETAILED = 2,      ///< \~chinese 对脸部做精细化磨皮处理 \~english high quality smooth on face
    EFFECT_SMOOTH_FACE_REFINE   = 3,      ///< \~chinese 对脸部皮肤细化和改善质感的磨皮处理 \~english refining and improving the texture of facial skin
    EFFECT_SMOOTH_FACE_EVEN     = 4,      ///< \~chinest 针对脸部皮肤平整的匀肤磨皮处理 \~english for the treatment of evening out and smoothing facial skin texture
...
/// \~chinese @brief 肤质模式
/// \~english @brief Skin texture mode
typedef enum {
    EFFECT_SKIN_TEXTURE_NONE     = 0,      ///< \~chinese 关闭肤质效果 \~english Disable skin texture effect
    EFFECT_SKIN_TEXTURE_CREAMY   = 1,      ///< \~chinese 奶油肌 \~english Creamy skin
    EFFECT_SKIN_TEXTURE_GLASSY   = 2,      ///< \~chinese 水光肌 \~english Glassy skin
    EFFECT_SKIN_TEXTURE_HD       = 3,      ///< \~chinese 高清肤质 \~english HD skin
    EFFECT_SKIN_TEXTURE_MATTE    = 4,      ///< \~chinese 雾面肌 \~english Matte skin
} st_effect_skin_texture_mode;

typedef enum {
    EFFECT_CONTRAST_DEFAULT = 0,       ///< \~chinese 默认对比度模式 \~english Default contrast mode
    EFFECT_CONTRAST_2       = 1        ///< \~chinese 对比度模式2 \~english Contrast mode 2
} st_effect_contrast_mode;

/// \~chinese
/// @brief 设置美颜的模式, 目前仅对磨皮、美白、背景虚化、肤质有效，具体支持的模式参考前面枚举值定义
/// @param[in] handle 已初始化的特效句柄
/// @param[in] param 美颜类型, 目前支持磨皮、美白、背景虚化
/// @param[in] mode 模式，目前可选择 st_effect_smooth_mode, st_effect_whiten_mode, st_effect_bokeh_mode, st_effect_skin_texture_mode
/// @return 成功返回ST_OK, 失败返回其他错误码, 错误码定义在st_mobile_common.h中, 如ST_E_FAIL等
/// \~english
...
typedef enum {
    ...
    EFFECT_BEAUTY_PARAM_ENABLE_BODY_SMOOTH,                     ///< \~chinese 修改美体参数时是否启用平滑效果，默认不启用。0 表示不启用，非0表示启用 \~english Whether to enable smoothing when changing body beautification parameters. Disabled by default. 0 means disabled, non-zero means enabled.
    EFFECT_BEAUTY_PARAM_ENABLE_BACKGROUND_WARP_COMPENSATION,    ///< \~chinese 背景扭曲优化开关，默认启用。0 表示关闭，非0表示启用 \~english Background warp suppression switch. Enabled by default. 0 means disabled, non-zero means enabled.
...
typedef enum {
    EFFECT_BODY_BEAUTIFY_LOCK_BACKGROUND = 0x01,   ///< \~chinese 美体时锁定背景 \~english Lock background when doing body beautify
    EFFECT_BODY_BEAUTIFY_BODY_MESH       = 0x10,   ///< \~chinese 侧身丰臀、侧身直背、瘦肚子 \~english Side hip enhancement, side straight back, slim belly
} st_effect_body_beautify_aux_type_t;

/// \~chinese
/// @brief 加载美体辅助资源，可以通过将path参数置为nullptr，清空之前类型设置的对应素材
/// @param[in] handle 已初始化的特效句柄
/// @param[in] param 美体辅助资源类型，支持按位组合
/// @param[in] path 待添加的文件路径
/// @return 成功返回ST_OK, 失败返回其他错误码, 错误码定义在st_mobile_common.h中, 如ST_E_FAIL等
/// \~english
/// @brief Load body beauty auxiliary resources, can clear previous corresponding material by setting the path parameter to nullptr.
/// @param[in] handle The initialized effects handle.
/// @param[in] param Body beauty auxiliary resource type, supports bitwise OR.
/// @param[in] path The file path of the auxiliary resource to be added.
/// @return Returns ST_OK if successful, otherwise returns other error codes. Error code definitions can be found in st_mobile_common.h, such as ST_E_FAIL etc.
ST_SDK_API st_result_t
st_mobile_effect_set_body_beauty(st_handle_t handle, st_effect_body_beautify_aux_type_t param, const char* path);

/// \~chinese
/// @brief 加载美体辅助资源
/// @param[in] handle 已初始化的特效句柄
/// @param[in] param 美体辅助资源类型，支持按位组合
/// @param[in] buffer 待添加的辅助资源缓存信息
/// @return 成功返回ST_OK, 失败返回其他错误码, 错误码定义在st_mobile_common.h中, 如ST_E_FAIL等
/// \~english
/// @brief Load body beauty auxiliary resources.
/// @param[in] handle The initialized effects handle.
/// @param[in] param Body beauty auxiliary resource type, supports bitwise OR.
/// @param[in] buffer The cache information of the material to be added.
/// @return Returns ST_OK if successful, otherwise returns other error codes. Error code definitions can be found in st_mobile_common.h, such as ST_E_FAIL etc.
ST_SDK_API st_result_t
st_mobile_effect_set_body_beauty_from_buffer(st_handle_t handle, st_effect_body_beautify_aux_type_t param, const st_effect_buffer_t* buffer);
...
/// @param[in] handle The initialized effects handle.
/// @param[in] p_package_path The file path of the material package to be used as the replacement.
...
/// @brief Replace the material package in the cache (delete the existing material package).
/// @param[in] handle The initialized effects handle.
/// @param[in] p_package_buffer The buffer information of the material package to be used as the replacement.
/// @param[out] p_package_id Material package ID.
/// @return Returns ST_OK if successful, otherwise returns other error codes. Error code definitions can be found in st_mobile_common.h, such as ST_E_FAIL etc.
ST_SDK_API st_result_t
...
/// \~chinese
/// @brief 替换缓存中的素材包 (删除package id的素材包)
/// @param[in] handle 已初始化的特效句柄
/// @param[in] old_package_id 将被替换的素材包package id
/// @param[in] p_package_path 待更换的素材包文件路径
/// @param[out] p_new_package_id 新素材包ID
/// @return 成功返回ST_OK, 失败返回其他错误码, 错误码定义在st_mobile_common.h中, 如ST_E_FAIL等
/// \~english
/// @brief Replace the material package in the cache (delete the material package with the package id).
/// @param[in] handle The initialized effects handle.
/// @param[in] old_package_id The package ID of the material package to be replaced.
/// @param[in] p_package_path The file path of the material package to be used.
/// @param[out] p_new_package_id New material package ID.
/// @return Returns ST_OK if successful, otherwise returns other error codes. Error code definitions can be found in st_mobile_common.h, such as ST_E_FAIL etc.
ST_SDK_API st_result_t
st_mobile_effect_replace_package(st_handle_t handle, int old_package_id, const char* p_package_path, int* p_new_package_id);
...
/// @param[in] old_package_id 将被替换的素材包package id
/// @param[in] p_package_buffer 待更换的素材包缓存信息
/// @param[out] p_new_package_id 素材包ID
/// @return 成功返回ST_OK, 失败返回其他错误码, 错误码定义在st_mobile_common.h中, 如ST_E_FAIL等
/// \~english
/// @brief Replace the material package in the cache (delete the material package with the package id).
/// @param[in] handle The initialized effects handle.
/// @param[in] old_package_id The package ID of the material package to be replaced.
/// @param[in] p_package_buffer The buffer information of the material package to be used.
/// @param[out] p_new_package_id New material package ID.
/// @return Returns ST_OK if successful, otherwise returns other error codes. Error code definitions can be found in st_mobile_common.h, such as ST_E_FAIL etc.
ST_SDK_API st_result_t
st_mobile_effect_replace_package_from_buffer(st_handle_t handle, int old_package_id, const st_effect_buffer_t* p_package_buffer, int *p_new_package_id);
...
typedef enum
{
    ...
    EFFECT_RESERVED_GAN_SERVER = 4,         ///< \~chinese ganserver request请求所需数据，类型是st_gan_request_t 或者 gan 请求的返回结果，类型是st_gan_return_t \~english Data required for ganserver request, the type is st_gan_request_t or the return result of gan request, the type is st_gan_return_t
...
ST_SDK_API st_result_t
st_mobile_effect_set_3d_beauty_parts_strength(st_handle_t handle, const st_effect_3D_beauty_part_info_t* parts, int partSize);
