...
typedef struct st_mobile_animal_face_t {
	...
	st_3dpose_t* p_pose3d;  ///< \~chinese 3D位置信息,方向与人正立方向一致，暂不支持resize mirror rotate后处理 \~english 3D position information, consistent with the direction of the person's upright position, not supported for resizing, mirroring, or rotation post-processing, currently
...
typedef struct st_mobile_animal_result_t
{
	...
	st_mobile_segment_t* p_animal_segments;        // 动物脸分割
	int segment_count;                             // 分割个数
...
typedef enum {
	...
	ST_MOBILE_ANIMAL_MODEL_SEGMENT = 20,         ///动物分割, animal segment
...
typedef enum {
	...
	ST_MOBILE_PARAM_ANIMAL_DELAY_FRAME = 4,
	...
	// 设置分割结果灰度图的方向是否需要旋转（0: 不旋转, 保持竖直; 1: 旋转, 方向和输入图片一致. 默认不旋转)
	ST_MOBILE_PARAM_ANIMAL_SEGMENT_ROTATE = 601,

	// 设置分割检测结果边缘模糊程度 取值范围0-1 视频版默认是0.5，图片版默认是1
	ST_MOBILE_PARAM_ANIMAL_SEGMENT_RESULT_BLUR = 602,
	...
	ST_MOBILE_PARAM_ANIMAL_CAM_FOVX = 5,

    /// \~chinese pose3d结果是否需要保持头朝上(1: detect输出的结果永远保持头朝上;  0: detect输出的pos方向和输入图片一致) 默认人头朝上 \~english Set whether the pose3D result should keep the head upright. The options are 1 (the pose3D result always keeps the head upright) and 0 (the pose3D result follows the same direction as the input image). By default, the head is kept upright
	ST_MOBILE_PARAM_ANIMAL_POSE3D_UP = 6,
...
#define ST_MOBILE_SEG_ANIMAL        0x00000020  ///< \~chinese 猫狗脸分割
